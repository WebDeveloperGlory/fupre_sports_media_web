...
export default function LiveMatchPage({
  ...
  return (
    <div className="min-h-screen bg-background selection:bg-emerald-500/30">
      ...
      <section className="relative pt-4 sm:pt-8 pb-4 overflow-hidden">
        ...
        <div className="container px-3 sm:px-4 mx-auto max-w-6xl space-y-3 sm:space-y-4">
          {/* Back Button */}
          <div className="flex items-center justify-between">
            <BackButton />
            <div className="text-center bg-card/60 backdrop-blur-sm rounded-full px-3 py-1 border border-border/50">
              <span className="text-xs font-medium text-muted-foreground">Live Viewers: </span>
              <span className="text-xs font-bold text-emerald-500">{liveWatchers || 0}</span>
            </div>
          </div>
          {
            liveFixture && (
                ...
                <div className='w-full overflow-x-scroll scrollbar-hide border rounded-lg flex md:grid md:grid-cols-2 items-center gap-2 bg-primary-foreground text-center'>
                  {
                    [liveFixture.homeTeam.name, liveFixture.awayTeam.name].map(tab => (
                      <div
                        ...
                        className={`
                              cursor-pointer px-6 py-2 capitalize text-sm font-medium basis-1/2 h-full ${activeModalTab === tab
                    ...
                    activeModalTab === liveFixture.homeTeam.name && (
                      [...liveFixture.lineups.home.startingXI, ...liveFixture.lineups.home.substitutes].map((player, i) => (
                        <div
                          ...
                          className={`
                              py-1 px-2 border rounded-md text-sm text-left cursor-pointer hover:border-emerald-500 flex justify-between items-center ${selectedPlayer === player.player._id
